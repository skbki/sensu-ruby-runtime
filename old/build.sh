...
do
    ...
    docker run -v "$PWD/dist:/dist" sensu-ruby-runtime-${ruby_version}-${platform}:${asset_version} cp /assets/sensu-ruby-runtime_${asset_version}_ruby-${ruby_version}_${platform}_linux_$(uname -m).tar.gz /dist/
done
