...
if [ -f "$PWD/dist/${asset_filename}" ]; then
  ...
  if [[ "$(docker images -q ${asset_image} 2> /dev/null)" == "" ]]; then
    ...
    docker buildx --platform "linux/arm64" --build-arg "RUBY_VERSION=$ruby_version" --build-arg "ASSET_VERSION=$asset_version" -t ${asset_image} -f Dockerfile.${platform} .
    echo "Making Asset: /assets/sensu-ruby-runtime_${asset_version}_ruby-${ruby_version}_${platform}_linux_$(uname -m).tar.gz"
    docker run --platform "linux/arm64" -v "$PWD/dist:/dist" ${asset_image} cp /assets/${asset_filename} /dist/
  #    #rm $PWD/test/*
