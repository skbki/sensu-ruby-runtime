...
if [ -f "$PWD/dist/${asset_filename}" ]; then
  ...
  if [[ "$(docker images -q ${asset_image} 2> /dev/null)" == "" ]]; then
    ...
    DOCKER_CLI_EXPERIMENTAL=enabled docker buildx build --load --platform "linux/arm64" --build-arg "RUBY_VERSION=$ruby_version" --build-arg "ASSET_VERSION=$asset_version" -t ${asset_image} -f Dockerfile.${platform} .
...
for test_platform in "${test_arr[@]}"; do
  ...
  docker run -e platform -e test_platform=${test_platform} -e asset_filename=${asset_filename} -v "$PWD/scripts/:/scripts" -v "/dist:/dist" ${test_platform} /scripts/test.sh
  retval=$?
  if [ $retval -ne 0 ]; then
